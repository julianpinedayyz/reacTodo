...
function TodoItem({ todo, onToggle, onDelete, onEdit, useOcticons = true }) {
  ...
  return (
    <li className={`relative flex items-center justify-between p-2.5 rounded-lg hover:bg-opacity-80 transition-colors duration-200 text-sm mb-3 ${
      ...
      <div className={`absolute top-1 left-2 flex items-center text-[0.65rem] ${themeClass('text-dracula-cyan', 'text-light-pink')}`}>
        {useOcticons ? (
      ...
      <div className="pt-5 flex-1 flex justify-between items-center"> {/* Fix layout to keep items aligned */}
        {isEditing ? (
          ...
          <>
            ...
            <div className="flex shrink-0"> {/* Make sure these buttons don't wrap */}
              <button
                ...
                className={`ml-1 w-8 h-8 flex items-center justify-center rounded-md transition-all duration-200 ${
                  themeClass(
